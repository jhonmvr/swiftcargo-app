import { Component, ElementRef, Input, OnInit, Renderer2, ViewChild, input } from '@angular/core';
...
export class MapComponent  implements OnInit {
  ...
  posSub: BehaviorSubject<any> = new BehaviorSubject<any>('');
  @Input() set pos(pos: any) {
    this.posSub.next(pos);
  }
  ...
  async ngAfterViewInit() {
    ...
    this.posSub.subscribe(async p=>{
     
    
      setTimeout(()=>{
        console.log("estos son los datos",p)
        if(!p){
          return
        }
        ...
        directionService.route({
          ...
          origin: new google.maps.LatLng(p.latInicio, p.lngInicio),
